...
from ....generated.models.query_expression import QueryExpression, QueryOperand, QueryOperator, Model
...
def is_negated_op(node: QueryOperand):
    res = (node.operator and "NOT" in node.operator) #or (node.prefix and "-" in node.prefix)
    return res
...
class AbstractSyntaxTreeVisitor(object):
    ...
    def visit(self, node: QueryExpression):
        ...
        return structure_operator(operator, node.left.accept(self), node.right.accept(self, is_negated))
    ...
    def visit(self, node: QueryOperand, is_negated: bool = False):
        if node.operator is not None:
            ...
            return structure_operator(operator, QueryOperand.accept(self, node.left, is_negated), QueryOperand.accept(self, node.right, is_negated))
