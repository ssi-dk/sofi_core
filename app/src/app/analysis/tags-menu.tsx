...
export const TagsMenu = (props: TagsMenuProps) => {
    ...
    const displayTags = [...allTags].filter(tag => !searchStr || tag.toLowerCase().includes(searchStr.toLowerCase()))
        .sort(
            (a, b) =>
                Number(workspace.tags.includes(b)) -
                Number(workspace.tags.includes(a))
        );
    ...
    return (
        <Menu>
            ...
            >
                ...
                {searchStr && <div style={{ width: "calc(100% - 1rem)",margin: "0.5rem",marginBottom: 0 }}><Button
                    onClick={() => setTag(searchStr.trim(), true)}
                    disabled={allTags.includes(searchStr.trim()) || setTagState.isPending}
                >
